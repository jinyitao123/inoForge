...
这是一份早期、待复核切片。它曾把采购订单审批后的待到货通知继续推进为“登记到货 → 来料检验 → 合格数量采购入库 → 库存余额与流水”，并以单行 PLC 订单验证到货 2、合格 1、入库 1。

2026-09-10 的 RISEMAP 同材料复核已确认，采购订单审批后产生的是一张含多条物料的订单级到货通知，不是每条订单明细一张通知。当前实现已迁移通知模型，因而本文件中的单物料登记验收不能再代表当前主线能力；多物料登记将在汇川 OTC 下一阶段重新办理和验收。
...
当前多物料到货登记尚未开放，旧动作已隐藏并在收到订单级通知时显式阻断。抽检方案、免检规则、批次追溯细化、供应商不合格处置、采购退货、应付与进项发票继续待同材料复核。ObjectStack 17.3.0 的动作事务仍受审计持久化超时影响，单据、库存和累计量采用顺序写入，尚无原子回滚保证。
