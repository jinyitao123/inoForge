...
2026-09-23 更新七应用归属与参考定位；既有设计审查只对应其原记录版本，本次调整尚未实施或验收。
...
Forge 按用户确认的七应用与业务合同建设原生能力。RISEMAP 为参考，客户选用外部系统后各项正式结果仍须有唯一归属。本文件专门维护跨系统能力组合；应用内设置、现有功能缺口治理和性能由产品总仓的 Forge 应用与性能设计维护。未实现或未验证项不因设计成立而成为可交付能力。
...
| 应用 | 可选择承办系统的业务能力 | 交接结果 |
| --- | --- | --- |
| 供应链 | 供应商、采购、到货质检、库存/出入库、物流 | 采购承诺、验收、正式库存变动与物流依据 |
| 销售 | 客户、商机、报价、合同、订单和售后 | 商务承诺、有效订单、变更与服务要求 |
| 生产 | 图纸/BOM、制造、装配、委外执行、过程质量 | 有效版本、进度、耗用、完工及质量结果 |
| 项目 | 立项、计划、任务、工时、现场交付与验收 | 计划、实际进展、投入、问题与交付成果 |
| 行政 | 员工服务、人事业务、考勤假期、差旅行政 | 员工事项与有效办理结果；账号岗位沿用平台 |
| 财务 | 应收应付、发票、收付款、核销、结算、会计确认 | 正式资金及结算状态、可追溯财务口径 |
| 报表 | 各正式系统授权数据与经营指标 | 标明来源/时效的分析，不能回写另一套业务账 |


应用是客户选择功能的层次，能力是确定工作交给谁的层次。例如客户可以用 Forge 管采购申请、ERP 管采购订单、QMS 管到货检验。能力按完整业务结果划分，避免把“新增”“保存”“审核”三个按钮分别交给不同系统。

跨应用动作遵守业务归属。生产应用提出领料要求，仓储系统确认实际扣账；销售或项目按交付责任提出发运要求，仓储系统确认出库；采购入库也不直接等于应付形成。总账与税务等能力只按明确交付范围接入或建设，本稿不承诺现有 Forge 可以替代完整 ERP。
...
| --- | --- | --- |
| 应用和业务页面 | 对象、动作、页面和应用装配 | 七个应用及内部能力边界、独立启用、订单与项目双入口 |
...
2026-09-12 原设计核对基线为本地主线 `2d0f303`，这是历史观察而非当前运行版本，`@objectstack/spec` 与运行时固定为 `17.3.0`。[入口配置](../apps/forge-objectstack/objectstack.config.ts)仍集中装配一个 Forge 应用；[库存动作](../apps/forge-objectstack/src/actions/inventory.action.ts)直接修改 Forge 库存余额与流水，采购等链路也存在内部直接关联。客户组合配置和业务能力替换尚未接入。当前多对象业务写入的恢复和岗位权限边界也需要随所选切片验证。
...
保持现有稳定记录标识和已经验证的业务行为，按链路替换跨模块直接依赖，逐条纳入统一交接，不要求一次搬迁全部数据库或拆成多个服务。Forge 原生能力按自身业务合同验收，RISEMAP 对照为可选参考；外部系统正式办理以客户实际规则为准，新增跨系统行为单独留证。当前授权范围为架构设计，本稿没有启动运行时重构。
