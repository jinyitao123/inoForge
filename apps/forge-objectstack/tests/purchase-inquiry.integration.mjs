...
await test('selects a quote and converts the exact line into a draft purchase order', async()=>{
  ...
  assert.deepEqual({supplier_id:order.supplier_id,total_amount:order.total_amount,status:order.status},{supplier_id:ids.supplier2,total_amount:13000,status:'draft'}); assert.deepEqual({order_id:line.order_id,quantity:line.quantity,taxed_unit_price:line.taxed_unit_price},{order_id:ids.order,quantity:2,taxed_unit_price:6500});
});
