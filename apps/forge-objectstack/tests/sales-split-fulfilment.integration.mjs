...
if (process.argv.includes('--readback')) {
  ...
  const next = await invoke('forge_sales_order','sales_order_create_shipment',ids.order,
    {code:'SPLIT-DN-2',shipment_on:'2026-09-09',recipient:'周启明',delivery_address:'苏州市工业园区澄岳路9号',quantity:1});
  ids.secondShipment=next.id;
  const out = await invoke('forge_sales_shipment','sales_shipment_create_outbound',next.id,
    {code:'SPLIT-OUT-2',warehouse_id:ids.warehouse,outbound_on:'2026-09-09',quantity:1});
  ...
  const second = await invoke('forge_sales_order','sales_order_issue_invoice',ids.order,
    {code:'SPLIT-INV-2',invoice_on:'2026-09-09',due_on:'2026-10-09',quantity:1});
  ...
  const over=await api.request(`/actions/forge_sales_order/sales_order_issue_invoice/${ids.order}`,'POST',
    {params:{code:'SPLIT-INV-OVER',invoice_on:'2026-09-09',due_on:'2026-10-09',quantity:1}});
...
assert.equal(balance.on_hand_quantity,1); assert.equal(balance.available_quantity,1); assert.equal(balance.inventory_value,58000);
