...
const endpoint=process.env.FORGE_URL||'http://localhost:4388',database=process.env.FORGE_DB||'.objectstack/otc-subcontract-inbound.sqlite';
...
await test('creates and submits a pending inbound without moving stock or order totals',async()=>{
  ...
  const submitted=await invoke('forge_subcontract_receipt','subcontract_receipt_submit',ids.receipt);assert.equal(submitted.status,200,JSON.stringify(submitted.value));const result=resultOf(submitted);ids.inbound=result.inbound_id;
...
await test('confirms once and posts finished stock, material backflush and order totals together',async()=>{
  ...
  assert.deepEqual({
    ...
    inboundLine:[inboundLine.status,inboundLine.material_amount,inboundLine.processing_amount,inboundLine.unit_cost,inboundLine.inventory_amount,inboundLine.before_on_hand,inboundLine.after_on_hand],
    ...
    balance:[balances.length,balances[0]?.on_hand_quantity,balances[0]?.available_quantity,balances[0]?.average_cost,balances[0]?.inventory_value],
  ...
  {receipt:'stocked',line:'stocked',consumption:['backflushed',12.5,75],inbound:['stocked',75,146,'fully_costed'],inboundLine:['stocked',75,71,73,146,0,2],stock:[0,6,0],backflush:[1,'backflush',6,75],order:['in_progress',3,6,0.18],orderLine:[2,1],plan:[6,0.18],balance:[1,2,2,73,146],ledger:[1,'subcontract_receipt_inbound',2,73,146],logs:['exception_recorded','inbound_created','stocked','submitted']});
});

const report={suite:'subcontract-inbound-confirm',endpoint,database,ids,cases,passed:cases.every(x=>x.status==='passed'),completedAt:new Date().toISOString(),result:{receipt:{received:3,qualified:2,defective:1},pendingBoundary:{supplierStock:6,orderReceived:0,formalStock:0},confirmed:{supplierStock:0,materialAmount:75,processingAmount:71,finishedQuantity:2,finishedUnitCost:73,finishedAmount:146,orderReceived:3}},risemapLiveEvidence:{routes:['/subcontract/receives/new','/subcontract/receives/:id','/inventory/subcontract-stock'],observed:['提交审核只生成待入库，不直接增加正式库存','确认入库后良品正式入库、材料实际耗用倒冲、订单累计同步更新','已完成入库不能撤回审核','成品入库和材料倒冲分别形成库存流水']},valuationBoundary:'RISEMAP 当前页面未直接展示成品计价公式；Forge 暂按每条回厂明细的实际材料成本加良品加工费形成成品入库成本。',boundary:'已闭环回厂提交到确认入库、正式库存、材料倒冲、订单累计和同库重启回读。不良 NCR、退料、补料、赔扣、对账和应付仍属后续切片。'};
