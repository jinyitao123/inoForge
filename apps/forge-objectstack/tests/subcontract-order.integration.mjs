...
const database = process.env.FORGE_DB || '.objectstack/otc-subcontract-order.sqlite';
const api = await connect(endpoint);
const cases = [], ids = { operator: api.userId };
...
ids.supplierCategory = await create('forge_supplier_category', { name: '委外加工供应商', code: 'SC-SUP-CAT', status: 'active' });
ids.supplierLevel = await create('forge_supplier_level', { name: '委外核心级', code: 'SC-SUP-LVL', status: 'active' });
ids.warehouseType = await create('forge_warehouse_type', { name: '委外仓库', code: 'SC-WH-TYPE', status: 'active' });
ids.unit = await create('forge_unit', { name: '件', code: 'PCS', status: 'active' });
ids.materialCategory = await create('forge_material_category', { name: '委外加工件', code: 'SC-MAT-CAT', status: 'active' });
ids.issueWarehouse = await create('forge_warehouse', { name: '委外发料仓', code: 'SC-WH-ISSUE', type_id: ids.warehouseType, responsible_id: api.userId, phone: '025-80000001', area: 200, address: '南京委外发料区' });
ids.receiptWarehouse = await create('forge_warehouse', { name: '委外回厂仓', code: 'SC-WH-RECEIPT', type_id: ids.warehouseType, responsible_id: api.userId, phone: '025-80000002', area: 180, address: '南京委外回厂区' });
ids.supplier = await create('forge_supplier', { name: '锐联钣金委外厂', code: 'SC-SUP-001', category_id: ids.supplierCategory, level_id: ids.supplierLevel, responsible_id: api.userId, payment_term: '月结 30 天', contact_name: '沈妍', phone: '13800000001', status: 'active' });
ids.unapprovedSupplier = await create('forge_supplier', { name: '未审批委外厂', code: 'SC-SUP-002', category_id: ids.supplierCategory, level_id: ids.supplierLevel, responsible_id: api.userId, payment_term: '月结 30 天', contact_name: '陈工', phone: '13800000002', status: 'active' });
...
await test('activates one governed subcontract supplier profile', async () => {
  const r = await invoke('forge_supplier', 'subcontract_supplier_activate', ids.supplier, { process_capabilities: '钣金折弯、喷涂', credit_rating: 'four', default_issue_warehouse_id: ids.issueWarehouse, default_receipt_warehouse_id: ids.receiptWarehouse, loss_rate_limit: 3, warranty_terms: '回厂验收后质保 12 个月' });
  ...
  assert.deepEqual({ supplier: profile.supplier_id, capability: profile.process_capabilities, rating: profile.credit_rating, issue: profile.default_issue_warehouse_id, receipt: profile.default_receipt_warehouse_id, status: profile.status, actor: profile.activated_by }, { supplier: ids.supplier, capability: '钣金折弯、喷涂', rating: 'four', issue: ids.issueWarehouse, receipt: ids.receiptWarehouse, status: 'active', actor: api.userId });
...
ids.finishedMaterial = await create('forge_material', { name: '电控柜门板', code: 'SC-FG-001', model: 'DB-1200', category_id: ids.materialCategory, unit_id: ids.unit, property: 'semi_finished', source_type: 'subcontracted', status: 'active', supplier_id: ids.supplier });
ids.finishedSku = await create('forge_material_sku', { name: '1200×600×2.0', code: 'SC-FG-SKU-001', material_id: ids.finishedMaterial, sale_price: 0, cost_price: 0, enabled: true });
ids.rawMaterial = await create('forge_material', { name: '冷轧钢板', code: 'SC-RM-001', model: 'SPCC', category_id: ids.materialCategory, unit_id: ids.unit, property: 'raw_material', source_type: 'purchased', status: 'active', supplier_id: ids.supplier });
ids.rawSku = await create('forge_material_sku', { name: '2.0mm', code: 'SC-RM-SKU-001', material_id: ids.rawMaterial, sale_price: 0, cost_price: 0, enabled: true });

ids.customerSuppliedOrder = await create('forge_subcontract_order', { name: '控制柜门板甲供料委外', code: 'SC-ORDER-001', supplier_profile_id: ids.profile, supplier_id: ids.supplier, supply_mode: 'customer_supplied', expected_delivery_on: '2026-09-30', source_type: 'manual', inspection_method: 'full', payment_term: '月结 30 天', responsible_id: api.userId });
...
ids.customerLine = await create('forge_subcontract_order_line', { name: '电控柜门板', order_id: ids.customerSuppliedOrder, sku_id: ids.finishedSku, item_code: 'SC-FG-001', specification: '1200×600×2.0', process_type: '钣金折弯、喷涂', quantity: 10, unit_name: '件', unit_price: 35.5, expected_delivery_on: '2026-09-30', drawing_number: 'DWG-SC-001' });
...
await test('derives customer-supplied totals and submits for approval', async () => {
  ...
  assert.deepEqual({ status: order.status, amount: order.processing_amount, total: order.total_quantity, issue: order.issue_planned_quantity, subtotal: line.subtotal, submitter: order.submitted_by }, { status: 'pending_approval', amount: 355, total: 10, issue: 20, subtotal: 355, submitter: api.userId });
});
...
ids.turnkeyOrder = await create('forge_subcontract_order', { name: '控制柜铭牌包工包料委外', code: 'SC-ORDER-002', supplier_profile_id: ids.profile, supplier_id: ids.supplier, supply_mode: 'turnkey', expected_delivery_on: '2026-10-05', source_type: 'manual', inspection_method: 'sampling', payment_term: '验收后 30 天', responsible_id: api.userId });
ids.turnkeyLine = await create('forge_subcontract_order_line', { name: '电控柜门板', order_id: ids.turnkeyOrder, sku_id: ids.finishedSku, item_code: 'SC-FG-001', specification: '1200×600×2.0', process_type: '激光打标', quantity: 5, unit_name: '件', unit_price: 12, expected_delivery_on: '2026-10-05', drawing_number: 'DWG-SC-002' });
