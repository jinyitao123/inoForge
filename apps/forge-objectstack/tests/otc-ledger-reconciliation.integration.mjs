...
const response = await api.request(
  `/actions/forge_project/project_reconcile_order_ledger/${project.id}`,
  'POST',
  { params: {} },
);
assert.equal(response.status, 200, JSON.stringify(response.value));

const result = response.value?.data || response.value?.result;
assert.deepEqual(
  {
    project: result.id,
    ...
    collectedAmount: result.collected_amount,
  },
  {
    project: project.id,
    orders: 1,
    contractAmount: 243200,
    invoiceAmount: 243200,
    collectedAmount: 243200,
  },
);
...
const refreshedProject = await one('forge_project', { id: project.id });
...
const invoiceLines = await find('forge_sales_invoice_line', { order_id: order.id });
...
assert.deepEqual(
  {
    ...
    project: [refreshedProject.contract_amount, refreshedProject.invoice_amount, refreshedProject.collected_amount],
    ...
    legacyInvoiceLineCount: invoiceLines.length,
  },
  {
    ...
    invoice: ['settled', 243200, 0],
    receivable: ['settled', 243200, 0],
    legacyInvoiceLineCount: 0,
  },
);

console.log(JSON.stringify({
  suite: 'otc-ledger-reconciliation',
  status: 'passed',
  endpoint,
  database,
  ...
  amount: 243200,
  note: '历史单行订单发票无明细，按有效发票金额反算已开票数量；未生成伪造发票明细。',
}, null, 2));
