...
assert.equal(report.passed, true, '连续委外链报告未通过');
...
const [receipt, inbound, consumption, stock, order, orderLine, plan, finishedBalances, stockLedgers, inventoryLedgers] = await Promise.all([
...
console.log(JSON.stringify({
  ...
  source_ids: { order: report.ids.customerSuppliedOrder, issue: report.ids.issue, receipt: report.ids.receipt, inbound: report.ids.inbound },
  assertion: '同一 SQLite 停服重启后按原单据 ID 回读委外链结果、库存、倒冲、流水和订单累计',
}, null, 2));
