...
function App(){
 ...
 const money=v=>'¥'+Number(v||0).toLocaleString('zh-CN',{minimumFractionDigits:2,maximumFractionDigits:2}),statusText={draft:'草稿',pending_approval:'待审批',approved:'已审批',stocked:'已入库',cancelled:'已取消'},sourceText={purchase_order:'采购订单',purchase_replacement:'采购换货补货'},inboundTypeText={purchase:'采购入库'},warehouseName=id=>state.warehouses.find(w=>w.id===id)?.name||'—',userName=id=>state.users?.[id]?.display_name||state.users?.[id]?.name||state.users?.[id]?.username||'Dev Admin';
...
<style>{css}</style>
<div className="body"><ForgeHero section="供应链 / 库存管理" title="采购入库" description="管理采购入库单据，并在审批后执行正式库存落账。" icon="▤" tone="blue" art="blueprint"/><div className="fp-action-row"><button className="fp-button primary" onClick={()=>window.location.href=forgeBase+'/page/page_purchase_inspection_workspace'}>新建采购入库</button><button className="fp-button" disabled={!filteredInbounds.length} onClick={()=>setPrintOpen(true)}>打印条码</button><button className="fp-button" disabled={!filteredInbounds.length} onClick={exportInbounds}>导出</button><button className="fp-button" onClick={()=>{setToast('');load();}}>刷新</button></div>{state.error&&<div className="notice">{state.error}</div>}{toast&&<div className="notice">{toast}</div>}<div className="card">
...
<option value="purchase_replacement">采购换货补货</option>
</ForgeSelectControl>
</div>
<div className="table">
<table>
...
<tr key={x.id}>
<td>{x.code}</td>
<td>{inboundTypeText[x.inbound_type]||'采购入库'}</td>
<td>{sourceText[x.source_type]||x.source_type||'采购订单'}</td>
<td>{inboundTypeText[x.inbound_type]||'采购入库'}</td>
...
</div>{printOpen&&<ForgeDialog open title="打印入库条码" subtitle={'共 '+filteredInbounds.length+' 张'} confirmLabel="打开打印" onCancel={()=>setPrintOpen(false)} onConfirm={printLabels}>
...
<tbody>{filteredInbounds.map(row=>
<tr key={row.id}>
<td>{row.code}</td>
