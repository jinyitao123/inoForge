...
<style>{css}</style>
<div className="header-shell"><ForgePageHeader badge="供" section="供应链 / 采购入库" title="新建入库单" description="入库单号将在保存时生成，提交审批后进入工作流。" actions={<><button className="fp-button" disabled={busy} onClick={()=>window.location.href='/_console/apps/com.inoforge.forge.supply-chain/page_purchase_inspection_workspace'}>返回检验单</button><button className="fp-button" disabled={busy} onClick={()=>create('draft')}>保存草稿</button><button className="fp-button primary" disabled={busy} onClick={()=>create('submit')}>提交审批</button></>}/></div>
<div className="flow">
