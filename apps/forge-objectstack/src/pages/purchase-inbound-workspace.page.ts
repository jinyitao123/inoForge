...
<style>{css}</style>
<div className="body"><ForgeHero section="供应链 / 库存管理" title="采购入库" description="管理采购入库单据，并在审批后执行正式库存落账。" icon="▤" tone="blue" art="blueprint"/><div className="fp-action-row"><button className="fp-button" disabled={!filteredInbounds.length} onClick={()=>setPrintOpen(true)}>打印条码</button><button className="fp-button" disabled={!filteredInbounds.length} onClick={exportInbounds}>导出</button><button className="fp-button" onClick={()=>{setToast('');load();}}>刷新</button><button className="fp-button primary" onClick={()=>window.location.href=forgeBase+'/page/page_purchase_inspection_workspace'}>新建入库单</button></div>{state.error&&<div className="notice">{state.error}</div>}{toast&&<div className="notice">{toast}</div>}<div className="card">
