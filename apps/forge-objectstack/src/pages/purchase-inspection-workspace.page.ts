...
function App(){
 ...
 async function runCreate(row){setBusy(row.id);try{const payload=await request('/actions/forge_pending_inspection/pending_inspection_create_order/'+row.id,{method:'POST',body:JSON.stringify({params:{inspection_method:methods[row.id]||'full'}})});const result=payload.result||payload.data?.result||payload.data||payload;if(!result.id)throw new Error('检验单创建后未返回记录ID');window.location.href=forgeBase+'/page/page_purchase_inspection_workspace?id='+encodeURIComponent(result.id);}catch(error){setDialog(d=>d?({...d,error:String(error.message||error)}):d);setState(s=>({...s,error:String(error.message||error)}));setBusy('');}}
...
<div className="body"><ForgePageHeader badge="供" section="供应链 / 质量管理" title="待检验库存" description="每个物料独立一条待检记录，关联来料检验流程。" actions={<button className="fp-button primary" onClick={()=>window.location.href=forgeBase+'/page/page_purchase_inspection_workspace'}>下一步：检验单</button>}/>{state.error&&<div className="notice">{state.error}</div>}{toast&&<div className="notice">{toast}</div>}<div className="card">
...
<style>{${JSON.stringify(sharedCss)}}</style>
<div className="body"><ForgePageHeader badge="供" section="供应链 / 质量管理" title="检验单" description="每条检验单对应一种物料的来料检验。" actions={<button className="fp-button primary" onClick={()=>window.location.href=forgeBase+'/page/page_purchase_inbound_workspace'}>下一步：采购入库</button>}/>{state.error&&<div className="notice">{state.error}</div>}{toast&&<div className="notice">{toast}</div>}<div className="card">
<div className="toolbar">
<button className="btn" disabled={!filteredRows.length} onClick={exportRows}>导出</button>
<button className="btn" onClick={()=>setTaskOpen(true)}>导入/导出任务</button>
<button className="btn" onClick={()=>{setToast('');load();}}>刷新</button>
