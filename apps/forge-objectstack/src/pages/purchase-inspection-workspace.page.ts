...
<td>{row.code}</td>
<td>{row.item_code}</td>
<td>{row.name.replace(/^PIN-\\d{4}-\\d+ /,'')}</td>
...
</span>:row.inspection_id?<button className="btn" onClick={()=>window.location.href='/_console/apps/forge/page/page_purchase_inspection_workspace?id='+encodeURIComponent(row.inspection_id)}>查看检验单</button>:'—'}</td>
</tr>)}</tbody>
...
<style>{${JSON.stringify(sharedCss)}}</style>
<div className="body"><ForgePageHeader badge="供" section="供应链 / 质量管理" title="检验单" description="每条检验单对应一种物料的来料检验。" actions={<button className="fp-button primary" onClick={()=>window.location.href='/_console/apps/forge/page/page_purchase_inbound_workspace'}>下一步：采购入库</button>}/>{state.error&&<div className="notice">{state.error}</div>}{toast&&<div className="notice">{toast}</div>}<div className="card">
<div className="toolbar">
<button className="btn" disabled={!filteredRows.length} onClick={exportRows}>导出</button>
<button className="btn" onClick={()=>setTaskOpen(true)}>导入/导出任务</button>
<button className="btn" onClick={()=>{setToast('');load();}}>刷新</button>
...
<div className="notice">{state.error||'检验单不存在'}</div>
 ...
 return <div className="forge-product forge-procurement forge-iqc">
<style>{${JSON.stringify(sharedCss)}}</style>
<div className="header-shell"><ForgePageHeader badge="供" section="供应链 / 质量管理" title={record.code} description={'来源 '+state.receipt.code+' · '+record.item_code+' · '+statusText[record.status]} actions={<><button className="fp-button" onClick={()=>window.location.href='/_console/apps/forge/page/page_pending_inspection_workspace'}>返回待检列表</button>{record.status==='pending'&&<button className="fp-button primary" disabled={busy} onClick={complete}>完成检验</button>}{record.status==='completed'&&Number(record.accepted_quantity||0)>0&&<button className="fp-button primary" onClick={()=>window.location.href='/_console/apps/forge/page/page_purchase_inbound_workspace?order='+encodeURIComponent(record.order_id)}>采购入库</button>}</>}/></div>
