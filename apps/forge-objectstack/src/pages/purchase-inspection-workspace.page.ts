...
<div className="body"><ForgeHero section="供应链 / 到货检验" title="待检验库存" description="每个物料独立一条待检记录，关联来料检验流程。" icon="▤" tone="orange" art="blueprint"/><div className="fp-action-row"><button className="fp-button primary" onClick={()=>window.location.href=forgeBase+'/page/page_purchase_inspection_workspace'}>下一步：检验单</button></div>{state.error&&<div className="notice">{state.error}</div>}{toast&&<div className="notice">{toast}</div>}<div className="card">
