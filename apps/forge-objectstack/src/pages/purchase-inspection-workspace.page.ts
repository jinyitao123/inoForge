...
function App(){const adapter=useAdapter();
 ...
 async function runCreate(row){setBusy(row.id);try{const payload=await request('/actions/forge_pending_inspection/pending_inspection_create_order/'+row.id,{method:'POST',body:JSON.stringify({params:{inspection_method:methods[row.id]||'full'}})});const result=payload.result||payload.data?.result||payload.data||payload;if(!result.id)throw new Error('检验单创建后未返回记录ID');navigate('/apps/com.inoforge.forge.supply-chain/page_purchase_inspection_workspace?id='+encodeURIComponent(result.id));}catch(error){setDialog(d=>d?({...d,error:String(error.message||error)}):d);setState(s=>({...s,error:String(error.message||error)}));setBusy('');}}
