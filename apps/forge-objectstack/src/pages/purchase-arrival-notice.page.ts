...
function App(){
 ...
 const order=id=>s.orders.find(x=>x.id===id),supplier=id=>s.suppliers.find(x=>x.id===id),wh=id=>s.warehouses.find(x=>x.id===id),lines=id=>s.lines.filter(x=>x.notice_id===id),today=new Date().toISOString().slice(0,10),statusText={pending_arrival:'待到货',partially_arrived:'部分到货',arrived:'已到货',cancelled:'已取消'},pct=x=>Number(x.planned_quantity)>0?Math.round(Number(x.arrived_quantity||0)/Number(x.planned_quantity)*100):0;
 ...
 function toggle(id){setSelected(v=>v.includes(id)?[]:[id])}function register(id){location.href='/_console/apps/forge/page/page_purchase_arrival_workspace?notice='+encodeURIComponent(id)}
 const css=\`${forgeProductUiCss}div:has(>.forge-an){max-width:none!important;margin:0!important}.forge-an+*{display:none}main:has(.forge-an)>div{padding:0!important}.forge-an{min-height:100%;background:#f3f5f7}.shell{padding:18px 22px 40px;max-width:1500px;margin:auto}.head{display:flex;justify-content:space-between;gap:14px;align-items:flex-start;margin-bottom:14px}.head h1{margin:0 0 5px;font-size:22px}.muted{color:#77818e}.toolbar,.filters,.actions,.pager{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.card{background:#fff;border:1px solid #dfe4e9;border-radius:7px;padding:15px}.filters{display:grid;grid-template-columns:minmax(230px,1fr) 155px 155px 190px 160px 150px;margin-bottom:12px}.filters input,.filters select{width:100%;box-sizing:border-box;border:1px solid #d4dae1;border-radius:4px;padding:8px}.btn{height:34px;border:1px solid #ccd3da;border-radius:4px;background:#fff;padding:0 10px}.primary{background:#1769e0;color:#fff;border-color:#1769e0}.btn:disabled{opacity:.45}.table{overflow:auto;border:1px solid #e1e5ea;border-radius:5px}.table table{width:100%;border-collapse:collapse;min-width:1100px;font-size:12px}.table th,.table td{padding:10px;border-bottom:1px solid #edf0f3;text-align:left}.table th{background:#f7f8fa;color:#68727f}.pill{padding:3px 8px;border-radius:10px;background:#edf4ff;color:#1769e0}.pager{justify-content:space-between;margin-top:12px}.notice{background:#fff3cd;color:#745600;padding:10px 12px;margin-bottom:12px;border-radius:5px}.detail{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.detail div{background:#f7f8fa;padding:10px;border-radius:4px}.detail small{display:block;color:#77818e}@media(max-width:900px){.filters{grid-template-columns:1fr}.head{display:block}.toolbar{margin-top:10px}}\`;
...
<td>{pct(x)}%<div className="muted">{x.arrived_quantity||0} / {x.planned_quantity||0}</div>
</td>
