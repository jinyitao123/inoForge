...
function App(){
 ...
 async function request(path){return ForgeApiRequest(adapter,path)}async function find(name){return(await request('/data/'+name+'?$top=200')).records||[]}
 ...
 function toggle(id){setSelected(v=>v.includes(id)?[]:[id])}function register(id){location.href='/_console/apps/com.inoforge.forge.supply-chain/page_purchase_arrival_workspace?notice='+encodeURIComponent(id)}
 const css=\`${forgeProductUiCss}div:has(>.forge-an){max-width:none!important;margin:0!important}.forge-an+*{display:none}main:has(.forge-an)>div{padding:0!important}.forge-an{min-height:100%;background:#f3f5f7}.shell{padding:18px 22px 40px;max-width:1500px;margin:auto}.head{display:flex;justify-content:space-between;gap:14px;align-items:flex-start;margin-bottom:14px}.head h1{margin:0 0 5px;font-size:22px}.muted{color:#77818e}.toolbar,.filters,.actions,.pager{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.card{background:#fff;border:1px solid #dfe4e9;border-radius:7px;padding:15px;container-type:inline-size}.filters{display:grid;grid-template-columns:minmax(230px,1fr) 155px 155px 190px 160px 150px;margin-bottom:12px}.filters>*{min-width:0}.filters>.fp-picker,.filters>.fp-date-picker{width:100%;min-width:0}.filters input,.filters select{width:100%;box-sizing:border-box;border:1px solid #d4dae1;border-radius:4px;padding:8px}.btn{height:34px;border:1px solid #ccd3da;border-radius:4px;background:#fff;padding:0 10px}.primary{background:#1769e0;color:#fff;border-color:#1769e0}.btn:disabled{opacity:.45}.table{max-width:100%;overflow:auto;scrollbar-width:thin;border:1px solid #e1e5ea;border-radius:5px;container-type:inline-size}.table table{width:100%;border-collapse:collapse;min-width:1100px;font-size:12px}.table th,.table td{padding:10px;border-bottom:1px solid #edf0f3;text-align:left}.table th:last-child{position:sticky;right:0;z-index:3;background:#f7f8fa;box-shadow:-1px 0 #e1e5ea}.table td:last-child:not([colspan]){position:sticky;right:0;z-index:2;background:#fff;box-shadow:-1px 0 #e1e5ea}.table td.arrival-empty-cell{position:sticky;left:0;width:100cqw;max-width:100%;text-align:center;background:#fff}.table th{background:#f7f8fa;color:#68727f;font-size:12px;padding:17px 12px;}.pill{padding:3px 8px;border-radius:10px;background:#edf4ff;color:#1769e0}.pager{justify-content:space-between;margin-top:12px}.notice{background:#fff3cd;color:#745600;padding:10px 12px;margin-bottom:12px;border-radius:5px}.detail{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.detail div{background:#f7f8fa;padding:10px;border-radius:4px}.detail small{display:block;color:#77818e}.anbar{height:6px;border-radius:3px;background:#e6ebf3;overflow:hidden;margin-bottom:4px}.anbar i{display:block;height:100%;background:#1d4ed8}@container (max-width:1120px){.filters{grid-template-columns:repeat(3,minmax(0,1fr))}.filters>:first-child{grid-column:span 2}.filters>.fp-picker,.filters>.fp-date-picker{min-width:0;width:100%}}@container (max-width:620px){.filters{grid-template-columns:repeat(2,minmax(0,1fr))}.filters>:first-child{grid-column:1/-1}}@container (max-width:460px){.filters{grid-template-columns:1fr}.filters>:first-child{grid-column:auto}}@media(max-width:900px){.head{display:block}.toolbar{margin-top:10px}}\`;
