...
function App(){const adapter=useAdapter();
 ...
 const checks={assembly:[
  ...
  ['生效 BOM',count('forge_bom',x=>x.status==='active')>0,count('forge_bom',x=>x.status==='active')+' 个生效版本',forgeBase+'/page/page_bom_workspace'],
  ['其他入库类型',count('forge_other_inbound_type',active)>0,count('forge_other_inbound_type',active)+' 个已启用类型',forgeBase+'/page/page_inventory_business_config'],
  ...
  ['委外供应商与工艺能力',count('forge_subcontract_supplier_profile',x=>x.status==='active'&&String(x.process_capabilities||'').trim())>0,count('forge_subcontract_supplier_profile',x=>x.status==='active'&&String(x.process_capabilities||'').trim())+' 家已开通',forgeBase+'/page/page_subcontract_suppliers'],
  ['已启用加工类型',count('forge_subcontract_business_setting',x=>x.category==='process_type'&&active(x))>0,count('forge_subcontract_business_setting',x=>x.category==='process_type'&&active(x))+' 个可选加工类型',forgeBase+'/page/page_subcontract_business_config'],
  ...
  ['当前加工价目',count('forge_subcontract_processing_price',x=>x.status==='active'&&(!x.effective_from||x.effective_from<=new Date().toISOString().slice(0,10))&&(!x.effective_to||x.effective_to>=new Date().toISOString().slice(0,10)))>0,count('forge_subcontract_processing_price',x=>x.status==='active')+' 条生效价目',forgeBase+'/page/page_subcontract_pricing'],
  ...
  ['按 BOM 展开发料',count('forge_bom',x=>x.status==='active')>0&&count('forge_bom_node',x=>x.is_leaf&&x.sku_id)>0,count('forge_bom',x=>x.status==='active')+' 个生效 BOM 可检查',forgeBase+'/page/page_bom_workspace'],
  ...
  ['退料原因',count('forge_subcontract_business_setting',x=>x.category==='return_reason'&&active(x))>0,count('forge_subcontract_business_setting',x=>x.category==='return_reason'&&active(x))+' 个可选原因',forgeBase+'/page/page_subcontract_business_config'],
  ['委外控制规则',count('forge_subcontract_policy',x=>x.status==='active')>0,count('forge_subcontract_policy',x=>x.status==='active')+' 套规则已生效',forgeBase+'/page/page_subcontract_policy'],
 ]};
