const bomPageSource = `
function App() {
  const [state, setState] = React.useState({ loading: true, boms: [], bom: null, nodes: [], logs: [], projects: {}, customers: {}, materials: {}, error: '' });
  ...
  async function load(preferredId) {
    try {
      ...
      const [nodeData, logData, projectData, customerData, materialData] = await Promise.all([
      ...
      setState({ loading: false, boms, bom, nodes: nodeData.records || [], logs: logData.records || [], projects, customers, materials, error: '' });
  ...
  return <div className="forge-bom"><style>{css}</style>
    ...
    {bom && <>
      ...
      {['缺料分析','应用/引用','图纸关联'].includes(tab) && <><div className="section-title">{tab}</div><div className="empty">该页签将按 RISEMAP 实测行为继续实现，当前不计入完成范围。</div></>}
      </main></div></>}
  </div>;
}
export default App;
