...
export const ProjectTimesheetSubmit = defineAction({
  ...
  body: { language: 'js', capabilities: ['api.write'], source: `const id=ctx.recordId||(ctx.record&&ctx.record.id),record=ctx.record;if(ctx.recordLoadDenied===true||!id||!record)throw new Error('工时记录不存在或不可访问');if(record.status!=='draft')throw new Error('仅草稿工时可以提交');await ctx.api.object('forge_project_timesheet').update({id,status:'pending_review',submitted_at:new Date().toISOString()});return{id,status:'pending_review'};` },
});
