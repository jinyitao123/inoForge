...
for(const input of requested){const line=lineMap[input.order_line_id];if(!line||seen.has(line.id))throw new Error('加工件不存在、重复或不属于当前订单');seen.add(line.id);const received=Number(input.received_quantity||0),good=Number(input.qualified_quantity||0),bad=Number(input.defective_quantity||0),remaining=round(Number(line.quantity||0)-Number(line.received_good_quantity||0)-Number(line.received_bad_quantity||0));if(!(received>0)||good<0||bad<0||round(good+bad)!==round(received))throw new Error((line.item_code||line.name)+' 必须满足本次回厂 = 合格数量 + 不良数量');if(received>remaining)throw new Error((line.item_code||line.name)+' 回厂数量超过订单剩余 '+remaining);const amount=round(good*Number(line.unit_price||0));prepared.push({line,received,good,bad,remaining,amount,batch:String(input.batch_number||'').trim()||null});total=round(total+received);goodTotal=round(goodTotal+good);badTotal=round(badTotal+bad);settlement=round(settlement+amount);}
...
const lines=await ctx.api.object('forge_subcontract_receipt_line').find({where:{receipt_id:id}});if(!lines.length)throw new Error('至少需要一行回厂验收明细');for(const line of lines){const orderLine=await ctx.api.object('forge_subcontract_order_line').findOne({where:{id:line.order_line_id}});if(!orderLine||orderLine.order_id!==record.order_id)throw new Error('回厂加工件与订单不一致');const received=Number(line.received_quantity||0),good=Number(line.qualified_quantity||0),bad=Number(line.defective_quantity||0),remaining=round(Number(orderLine.quantity||0)-Number(orderLine.received_good_quantity||0)-Number(orderLine.received_bad_quantity||0));if(!(received>0)||round(good+bad)!==round(received)||received>remaining)throw new Error((line.item_code||line.name)+' 数量关系或订单剩余量已变化');}
