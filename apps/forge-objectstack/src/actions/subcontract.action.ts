...
const consumptions=await ctx.api.object('forge_subcontract_receipt_consumption').find({where:{receipt_id:id}}),prepared=[],stockClaims={};if(order.supply_mode==='customer_supplied'&&!consumptions.length)throw new Error('甲供料回厂缺少材料耗用');
for(const item of consumptions){const plan=await ctx.api.object('forge_subcontract_material_plan').findOne({where:{id:item.plan_id}});if(!plan||plan.order_id!==record.order_id)throw new Error('材料耗用计划与订单不一致');const stocks=await ctx.api.object('forge_subcontract_stock_balance').find({where:{balance_key:record.supplier_id+':'+record.order_id+':'+item.sku_id}});if(stocks.length!==1)throw new Error((item.item_code||item.name)+' 委外厂库存余额不存在或重复');const stock=stocks[0],actual=Number(item.actual_quantity||0);stockClaims[stock.id]=round((stockClaims[stock.id]||0)+actual);if(stockClaims[stock.id]>Number(stock.on_hand_quantity||0))throw new Error((item.item_code||item.name)+' 委外厂在外库存不足');prepared.push({item,plan,stock,actual});}
const now=new Date().toISOString();let materialAmount=0;for(let index=0;index<prepared.length;index++){const x=prepared[index],stock=await ctx.api.object('forge_subcontract_stock_balance').findOne({where:{id:x.stock.id}}),before=Number(stock.on_hand_quantity||0),unitCost=before>0?round(Number(stock.inventory_value||0)/before):Number(stock.unit_cost||0),amount=round(x.actual*unitCost),after=round(before-x.actual),value=round(Math.max(0,Number(stock.inventory_value||0)-amount));if(x.actual>0){await ctx.api.object('forge_subcontract_stock_balance').update({id:stock.id,backflushed_quantity:round(Number(stock.backflushed_quantity||0)+x.actual),on_hand_quantity:after,inventory_value:value,last_movement_at:now});await ctx.api.object('forge_subcontract_stock_ledger').insert({name:record.code+' '+x.item.name+' 回厂倒冲',code:record.code+'-BF-'+String(index+1).padStart(3,'0'),supplier_id:record.supplier_id,order_id:record.order_id,sku_id:x.item.sku_id,direction:'outbound',movement_type:'backflush',quantity:x.actual,before_on_hand:before,after_on_hand:after,unit_cost:unitCost,amount,occurred_at:now,source_object:'forge_subcontract_receipt',source_id:id,source_line_id:x.item.id,responsible_id:actor,remarks:'回厂验收按 BOM 记录实际耗用'});}await ctx.api.object('forge_subcontract_material_plan').update({id:x.plan.id,backflushed_quantity:round(Number(x.plan.backflushed_quantity||0)+x.actual),overconsumption_quantity:round(Number(x.plan.overconsumption_quantity||0)+Number(x.item.overconsumption_quantity||0))});await ctx.api.object('forge_subcontract_receipt_consumption').update({id:x.item.id,unit_cost:unitCost,amount,status:'backflushed'});materialAmount=round(materialAmount+amount);}
let inboundId=null;if(Number(record.qualified_quantity||0)>0){const saved=await ctx.api.object('forge_subcontract_inbound').insert({name:record.code+' 待入库',code:'IN-'+record.code,receipt_id:id,order_id:record.order_id,supplier_id:record.supplier_id,warehouse_id:record.warehouse_id,total_quantity:Number(record.qualified_quantity||0),processing_amount:Number(record.settlement_amount||0),valuation_status:'processing_only',status:'pending',created_by:actor,created_at_business:now,remarks:'材料成本已回冲；完整成品成本和正式库存由后续入库环节核定'});inboundId=typeof saved==='string'?saved:saved&&(saved.id||(saved.record&&saved.record.id));for(const line of lines){if(Number(line.qualified_quantity||0)<=0)continue;await ctx.api.object('forge_subcontract_inbound_line').insert({name:line.name,inbound_id:inboundId,receipt_line_id:line.id,order_line_id:line.order_line_id,sku_id:line.sku_id,warehouse_id:line.warehouse_id,item_code:line.item_code,specification:line.specification||null,unit_name:line.unit_name,qualified_quantity:Number(line.qualified_quantity||0),batch_number:line.batch_number||null,processing_unit_price:Number(line.processing_unit_price||0),processing_amount:Number(line.settlement_amount||0),status:'pending'});}}
for(const line of lines){const orderLine=await ctx.api.object('forge_subcontract_order_line').findOne({where:{id:line.order_line_id}});await ctx.api.object('forge_subcontract_order_line').update({id:orderLine.id,received_good_quantity:round(Number(orderLine.received_good_quantity||0)+Number(line.qualified_quantity||0)),received_bad_quantity:round(Number(orderLine.received_bad_quantity||0)+Number(line.defective_quantity||0))});await ctx.api.object('forge_subcontract_receipt_line').update({id:line.id,status:Number(line.qualified_quantity||0)>0?'pending_inbound':'inspection_exception'});}
const received=round(Number(order.received_quantity||0)+Number(record.total_received_quantity||0)),next=received>=Number(order.total_quantity||0)?'completed':'in_progress';await ctx.api.object('forge_subcontract_order').update({id:order.id,received_quantity:received,backflushed_quantity:round(Number(order.backflushed_quantity||0)+Number(record.actual_material_quantity||0)),overconsumption_quantity:round(Number(order.overconsumption_quantity||0)+Number(record.overconsumption_quantity||0)),status:next});const receiptStatus=inboundId?'pending_inbound':'inspection_exception';await ctx.api.object('forge_subcontract_receipt').update({id,status:receiptStatus,inbound_id:inboundId,submitted_by:actor,submitted_at:now});await ctx.api.object('forge_subcontract_receipt_log').insert({name:record.code+' 提交回厂验收',receipt_id:id,action:'submitted',from_status:'draft',to_status:receiptStatus,comment:'记录材料倒冲、良品与不良品验收结果',occurred_at:now,operator_id:actor});if(inboundId)await ctx.api.object('forge_subcontract_receipt_log').insert({name:record.code+' 生成待入库单',receipt_id:id,action:'inbound_created',from_status:receiptStatus,to_status:receiptStatus,comment:'合格品 '+record.qualified_quantity+' 件进入待入库，尚未增加正式库存',occurred_at:now,operator_id:actor});if(Number(record.defective_quantity||0)>0)await ctx.api.object('forge_subcontract_receipt_log').insert({name:record.code+' 登记不良品',receipt_id:id,action:'exception_recorded',from_status:receiptStatus,to_status:receiptStatus,comment:'不良品 '+record.defective_quantity+' 件待后续 NCR 处理',occurred_at:now,operator_id:actor});return{id,status:receiptStatus,inbound_id:inboundId,qualified_quantity:Number(record.qualified_quantity||0),defective_quantity:Number(record.defective_quantity||0),settlement_amount:Number(record.settlement_amount||0),material_amount:materialAmount,formal_stock_added:0};
` },
});
