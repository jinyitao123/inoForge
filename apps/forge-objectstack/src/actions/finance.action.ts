...
for(const receivable of receivables)await ctx.api.object('forge_accounts_receivable').update({id:receivable.id,outstanding_amount:0,status:'red_reversed',remarks:(receivable.remarks||'')+'；原发票已红冲 '+ctx.input.code});await ctx.api.object('forge_sales_invoice').update({id,status:'red_reversed',outstanding_amount:0,reversed_by:actor,reversed_at:now,reversal_reason:reason});await ctx.api.object('forge_invoice_reversal_log').insert({name:invoice.code+' 销项红冲',event_key:invoice.code+'-RED-REVERSE',direction:'sales',original_sales_invoice_id:id,red_sales_invoice_id:redId,original_purchase_invoice_id:null,red_purchase_invoice_id:null,amount:total,reason,occurred_at:now,operator_id:actor});return{id,red_invoice_id:redId,status:'red_reversed',order_id:order.id,amount:total,order_invoiced_amount:nextOrder};
` },
});
