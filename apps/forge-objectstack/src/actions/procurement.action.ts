...
if(mode==='submit'){
  ...
  for(const row of inspectRows){const item=row.item;pendingSeq+=1;const pendingCode='PIN-'+year+'-'+String(pendingSeq).padStart(4,'0');await ctx.api.object('forge_pending_inspection').insert({name:pendingCode+' '+item.noticeLine.name,code:pendingCode,receipt_id:receiptId,receipt_line_id:item.receiptLineId,order_id:order.id,order_line_id:item.orderLine.id,supplier_id:notice.supplier_id,customer_id:ctx.input.customer_id||null,warehouse_id:item.warehouseId,sku_id:item.noticeLine.sku_id,item_code:item.noticeLine.item_code,model:item.noticeLine.model,specification:item.noticeLine.specification,unit_name:item.noticeLine.unit_name,arrival_quantity:item.quantity,batch_number:item.input.batch_number||null,external_sn:item.input.external_sn||null,arrived_on:ctx.input.arrived_on,status:'pending',responsible_id:actor,remarks:'由到货登记 '+code+' 按检验方案 '+((row.plan&&row.plan.code)||'')+' 提交待检'});await ctx.api.object('forge_purchase_receipt_line').update({id:item.receiptLineId,status:'pending_inspection'});}
