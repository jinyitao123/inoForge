...
const orderInbound=round4(Number(order.inbound_quantity||0)+total),orderStatus=orderInbound>=Number(order.total_quantity||0)?'completed':'partially_arrived';await ctx.api.object('forge_purchase_order').update({id:order.id,inbound_quantity:orderInbound,status:orderStatus});await ctx.api.object('forge_purchase_inbound').update({id,status:'stocked',stocked_at:now,stocked_by:actor});await ctx.api.object('forge_purchase_inbound_approval_log').insert({name:inbound.code+' 执行入库',inbound_id:id,action:'stocked',from_status:'approved',to_status:'stocked',comment:'采购入库完成',occurred_at:now,operator_id:actor});return{id,status:'stocked',line_count:lines.length,total_quantity:total,order_status:orderStatus};
`},
});
